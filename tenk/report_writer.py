...
# tenk 報告輸出根目錄（純 markdown + raw json，不再產 PDF / 圖表）
BASE_DIR = Path(Config.TENK_OUTPUT_DIR)
...
def save_report(ticker, results, eval_results, synthesis, quarterly=None,
    ...
    # ── Save raw JSON（供 bot 抽 telegram 摘要用） ──
    ...
    logger.info(f"[tenk] 報告：{report_md}")
    logger.info(f"[tenk] 原始 JSON：{json_path}")
    return report_md


# ────────────────────────────────────────────
# Telegram 短摘要（供 bot 直接推送，HTML safe）
# ────────────────────────────────────────────
...
def build_telegram_summary(
    ...
    """組成 4096 字以內的 Telegram HTML 摘要。完整內容仍存於 markdown 報告。"""
    insight = synthesis.get("insight", {}) or {}
    rerate = results.get("rerate_signal", {}) or {}

    label = filing_type + (f" {quarter}" if quarter else "")
    parts = [f"📑 <b>{_h(ticker)} {_h(label)} 深度分析</b>", ""]

    # 紅綠燈
    rerate_items = [
        ("structure_changing", "結構在變"),
        ("quality_changing", "品質在變"),
        ("narrative_changing", "敘事在變"),
    ]
    if filing_type == "10-Q":
        rerate_items = [it for it in rerate_items if it[0] != "structure_changing"]

    if rerate and "error" not in rerate:
        light_strs = []
        for key, lbl in rerate_items:
            cond = rerate.get(key, {}) or {}
            if cond.get("result"):
                icon = "🟢"
            elif cond.get("emerging"):
                icon = "🟡"
            else:
                icon = "🔴"
            light_strs.append(f"{icon}{_h(lbl)}")
        if light_strs:
            parts.append("<b>評價趨勢</b>　" + "　".join(light_strs))
            parts.append("")

    # Bull case 前 3 點
    bull = insight.get("bull_case") or []
    if bull:
        parts.append("🟢 <b>多頭論點</b>")
        for item in bull[:3]:
            point = item.get("point", "") if isinstance(item, dict) else str(item)
            parts.append(f"• {_h(point)}")
        parts.append("")

    # Bear case 前 3 點
    bear = insight.get("bear_case") or []
    if bear:
        parts.append("🔴 <b>空頭論點</b>")
        for item in bear[:3]:
            point = item.get("point", "") if isinstance(item, dict) else str(item)
            parts.append(f"• {_h(point)}")
        parts.append("")
    ...
    # 關鍵追蹤指標
    monitorables = insight.get("key_monitorables") or []
    if monitorables:
        parts.append("🎯 <b>關鍵追蹤指標</b>")
        for item in monitorables[:5]:
            text = item if isinstance(item, str) else item.get("metric", "") if isinstance(item, dict) else str(item)
            parts.append(f"• {_h(text)}")
        parts.append("")

    parts.append("📎 完整 markdown 報告隨後送出")
