...
async def eval_single(task_id, skill_output, source_input) -> dict:
...
async def eval_all(results, sections, filing_type: str = "10-K", quarter=None) -> dict:
    ...
    eligible = [
        (task_id, output, source_map.get(task_id, ""))
    ...
    async def _bounded(task_id, output, source):
        async with sem:
            ev = await eval_single(task_id, output, source)
            status = "PASS" if ev.get("pass") else "FAIL"
            logger.info(f"    eval {task_id}: {status} ({ev.get('total', '?')})")
            return task_id, ev

    pairs = await asyncio.gather(
        *[_bounded(tid, out, src) for tid, out, src in eligible]
    )
    return dict(pairs)
